  ...
  static adminGetSEOProfile = async (
    ...
    try {
      const res = await axios.get<AdminSEORes>(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/v1/admin/model/seo-list?limit=${limit}&offset=${offset}&search_field=${search_field}&sort_field=${sort_field}&sort_order=${sort_order}`,
        {
          headers: {
            'Content-Type': 'application/json',
            Authorization: token
